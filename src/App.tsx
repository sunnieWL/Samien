...
type Store = { version: 4; records: Record<string, RecordState>; rescheduleRequests: RescheduleRequest[]; gmailConnected: boolean }
...
const fullStore = (): Store => ({ version: 4, records: Object.fromEntries(documents.map(doc => [doc.id, defaultRecord()])), rescheduleRequests: [], gmailConnected: false })
const emptyStore = (): Store => ({ version: 4, records: {}, rescheduleRequests: [], gmailConnected: false })
function readStore(): Store {
  try {
    const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null') as Store | null
    if (parsed?.version === 4 && parsed.records && typeof parsed.records === 'object') return { ...parsed, rescheduleRequests: Array.isArray(parsed.rescheduleRequests) ? parsed.rescheduleRequests : [], gmailConnected: parsed.gmailConnected === true }
...
function Sidebar({ section, setSection, records, collapsed, setCollapsed, gmailConnected, onConnectGmail, onResetDemo }: { section: Section; setSection: (s: Section) => void; records: Store['records']; collapsed: boolean; setCollapsed: (v: boolean) => void; gmailConnected: boolean; onConnectGmail: () => void; onResetDemo: () => void }) {
  ...
  return <>
    ...
    <ProfileSettings open={profileOpen} onOpenChange={setProfileOpen} gmailConnected={gmailConnected} onConnectGmail={onConnectGmail} onResetDemo={onResetDemo} />
  </>
}
...
function AppShell({ section, setSection, records, onScan, gmailConnected, onConnectGmail, onResetDemo, children }: { section: Section; setSection: (s: Section) => void; records: Store['records']; onScan: () => void; gmailConnected: boolean; onConnectGmail: () => void; onResetDemo: () => void; children: React.ReactNode }) {
  const [menuOpen, setMenuOpen] = useState(false)
  return <div className="app-shell"><Sidebar section={section} setSection={setSection} records={records} collapsed={menuOpen} setCollapsed={setMenuOpen} gmailConnected={gmailConnected} onConnectGmail={onConnectGmail} onResetDemo={onResetDemo} /><div className="app-body"><Topbar section={section} onScan={onScan} onMenu={() => setMenuOpen(true)} />{children}</div></div>
}
...
function App() {
  ...
  const reset = (empty: boolean) => {
    setStore(empty ? emptyStore() : fullStore())
    setSelected([])
    setActiveId(null)
    setSearch('')
    setArchiveType('all')
